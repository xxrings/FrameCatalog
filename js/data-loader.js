...
function inferType(frame, materials) {
  const normalizedMaterials = materials.map(material => material.toLowerCase());
  if (frame.safety || normalizedMaterials.includes('safety')) return 'Safety';
  if (frame.sport || normalizedMaterials.includes('sport')) return 'Sport';
  return 'Standard';
}

function normalizeFrame(frame) {
  const frameName = normalizeName(frame.FrameName);
  const material1 = normalizeName(frame.Material1 || frame.Material);
  const material2 = normalizeName(frame.Material2);
  const materials = [...new Set([material1, material2].filter(Boolean))];
  ...
  return {
    ...
    type: inferType(frame, materials)
  };
}
